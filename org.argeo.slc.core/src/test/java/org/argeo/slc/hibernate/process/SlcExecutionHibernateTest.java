...
public class SlcExecutionHibernateTest extends AbstractSpringTestCase {
	...
	public void testModify() {
		...
		List<String> logLineListStep0 = slcExecRetrieved.getSteps().get(0).getLogLines();
		...
		SlcExecutionTestUtils.assertSlcExecution(slcExecRetrieved, slcExecUpdated);
		assertEquals(3, slcExecUpdated.getSteps().size());
		SlcExecutionTestUtils.assertSlcExecutionStep(slcExecUpdated.getSteps().get(0),
				slcExecRetrieved.getSteps().get(0));
		SlcExecutionTestUtils.assertSlcExecutionStep(slcExecUpdated.getSteps().get(1),
				slcExecRetrieved.getSteps().get(1));
		SlcExecutionTestUtils.assertSlcExecutionStep(slcExecUpdated.getSteps().get(2),
				slcExecRetrieved.getSteps().get(2));
	}
